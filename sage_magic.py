...
try:
    ...
    def ask(line, cell=None):
        ...
        system_prompt_text = (
            ...
            f"REGISTRY PURPOSE — ALWAYS register every named layer or class you color in "
            f"`.sage_colors.json`, EVEN single-class layers (one entry, one color). The registry "
            f"is the source of truth for color continuity ACROSS CELLS, not just for distinguishing "
            f"classes within a single cell. Before assigning a new color to a layer/class, FIRST "
            f"load `.sage_colors.json` and check whether that field already has an entry — if so, "
            f"reuse the existing color so the same entity keeps the same color throughout the "
            f"notebook. If absent, pick a color AND write the new entry back. A later cell rendering "
            f"'rivers' must find the same blue an earlier cell chose, not pick a fresh one.\n"
