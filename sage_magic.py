...
try:
    ...
    def ask(line, cell=None):
        ...
        system_prompt_text = (
            ...
            f"(2) /home/jovyan/.deepagents/agent/skills/ — read-only access to skill files "
            ...
            f"even as a 'let me try' fallback when the right key is missing. Sending the wrong key "
            f"to the wrong URL leaks the secret to that site's transport logs / monitoring / staff, "
            f"even if auth correctly fails. Specific forbidden patterns:\n"
            f"  • `NRP_API_KEY` (or any `*_API_KEY` for an LLM provider like ANTHROPIC, OPENAI, GOOGLE) "
            f"→ any non-LLM endpoint (OpenTopography, Earthdata, AWS, Google services, "
            f"Planetary Computer, etc.)\n"
            ...
            f"On a missing service-specific key: STOP, tell the user which env var they need to set "
            f"and where to obtain it (e.g. `portal.opentopography.org` → My Account → Request API key), "
            f"and HALT until they set it. Do NOT substitute another service's credential. Do NOT "
            f"silently switch to an alternative data source without flagging the credential gap. "
