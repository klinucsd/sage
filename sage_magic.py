...
try:
    ...
    def ask(line, cell=None):
        ...
        full_prompt = (
            ...
            f"color encodes class membership, NOT individual identity. If the entities can be "
            f"grouped into a small number of meaningful classes (e.g., 'airport' vs 'national "
            f"park', 'state' vs 'county', 'observed' vs 'predicted', 'forest' vs 'grassland'), "
            f"use ONE color per CLASS and distinguish individuals via labels, tooltips, or "
            f"marker shapes. Do NOT assign a unique color to each individual entity unless the "
            f"user explicitly asks for per-entity coloring (e.g., 'show each region in a "
            f"different color'). A legend with more than ~5–7 colors is a sign you've chosen "
            f"the wrong color encoding — regroup by class first.\n"
