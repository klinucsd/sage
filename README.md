...
- **Installed from a local path or a GitHub URL** with the `%%skill` cell magic — paste paths or `github.com/<org>/<repo>/tree/<commit>/...` URLs and ARGUS loads them into the kernel. Skills from allowlisted GitHub orgs install silently; others are surfaced in a single trust prompt with `SKILL.md` descriptions for review
